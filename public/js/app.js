...
var UpsertTips = {
  ...
  save: function save() {
    ...
    axios({
      ...
      var modal = bootstrap.Modal.getInstance(UpsertTips.modal);
      ...
      if (error.response.data.errors.hasOwnProperty('type_vehicle')) {
        UpsertTips.setErrorInvalid(UpsertTips.typeVehicleError, UpsertTips.typeVehicle, error.response.data.errors.type_vehicle);
      } else {
        UpsertTips.setValid(UpsertTips.typeVehicle);
      }

      if (error.response.data.errors.hasOwnProperty('brand')) {
        UpsertTips.setErrorInvalid(UpsertTips.marcaVehicleInputError, UpsertTips.marcaVehicleInput, error.response.data.errors.brand);
      } else {
        UpsertTips.setValid(UpsertTips.marcaVehicleInput);
      }

      if (error.response.data.errors.hasOwnProperty('model')) {
        UpsertTips.setErrorInvalid(UpsertTips.modeloVehicleError, UpsertTips.modeloVehicle, error.response.data.errors.model);
      } else {
  ...
  showAlert: function showAlert(errorArray) {
    var alertVehicleTips = document.createElement('div');
    var alertButton = document.createElement('button');
    var html = '';
    errorArray.forEach(function (message) {
      html += "".concat(message, "<br>");
